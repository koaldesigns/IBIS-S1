...
//debugPrintf("psi = %.2f, theta = %.2f\n", psi * RadiansToDegrees, theta * RadiansToDegrees);
	...
	/* wristMode is defined as the movement type for the wrist joint.
	0 = same orientation as distal arm
	1 = definable position in relation to x-axis by user
...
bool ScaraKinematics::Configure(unsigned int mCode, GCodeBuffer& gb, const StringRef& reply, bool& error) THROWS(GCodeException) /*override*/
{
	if (mCode == 669)
	{
		...
		2 = same orientation as x-axis
		*/
		gb.TryGetFValue('W', wristMode, seen);
		...
		if (gb.TryGetFloatArray('E', 2, betaLimits, reply, seen))		//betaLimits is defined as the min and max angle of the wrist joint
		{
			error = true;
			return true;
		}		
...
void ScaraKinematics::OnHomingSwitchTriggered(size_t axis, bool highEnd, const float stepsPerMm[], DDA& dda) const noexcept
{
	...
	case 3:		// wrist joint homing switch
		{
			const float hitPoint = (highEnd) ? betaLimits[1] : betaLimits[0];
			dda.SetDriveCoordinate(lrintf(hitPoint * stepsPerMm[axis]), axis);
		}
		break;
